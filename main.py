...
class MyMainWindow(QMainWindow,Ui_MainWindow):
    def __init__(self,parent=None):
        ...
        self.setupUi(self)
        self.SCR_1.verticalScrollBar().setValue(self.SCR_1.verticalScrollBar().maximum())
        ...
        self.qwqDlg=[QMessageBox(QMessageBox.Icon.NoIcon,"QwQ","QwQ",QMessageBox.StandardButton.Ok,self) for _ in range(18)]
        for dlg in self.qwqDlg:
            dlg.setModal(False)

    def fx(self,x):
        self.menu.exec(self.mapToGlobal(QPoint(0,self.menubar.height()))+self.WID_1.pos()+x)
        
    def f1(self,x):
        self.CBB_1.setText(["no","yes"][int(bool(x))])
    
    def f2(self):
        QMessageBox.information(self,"","You wouldn't get get this from any other guy")
    ...
    def f8(self):
        QFileDialog.getOpenFileName(self,"Open File","data/","All Files (*.*);;Don't (*tell*);;me (*you're*);;too (*blind*);;to (*see*)")
    ...
    def f12(self,x):
        for i,dlg in enumerate(self.qwqDlg):
            dlg.move(self.mapToGlobal(QPoint(self.width()//2+i*20,self.height()//2+i*20)))
            dlg.show()
            time.sleep(0.05)
